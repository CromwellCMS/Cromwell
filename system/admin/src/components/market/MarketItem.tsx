...
export default function MarketItem(props: PropsType) {
  ...
  const [installed, setInstalled] = useState(
    !!(props.data?.name && props?.listItemProps?.installedModules?.find((inst) => inst.name === props.data?.name)),
  );
  ...
  return (
    <Grid item xs={6} lg={4} className={styles.listItem}>
      <div className={clsx(styles.listItemContent, installing && styles.installing)}>
        {data?.image && (
          <CardActionArea onClick={() => props.listItemProps?.open(props.data)} className={styles.cardActionArea}>
        ...
        {installing && <LinearProgress className={styles.updateProgress} />}
      </div>
    </Grid>
  );
}
