...
export const InnerPageFrame = () => {
  const { editingPageConfig, pageFrameRef, forceUpdate } = useThemeEditor();
      ...
      <div className="h-full w-full relative">
        ...
        <div className="mx-auto max-w-lg my-20">
          ...
          <p className="my-10 text-center">
            This is the place where you can add new custom pages to your website, edit existing pages - or even change
            the theme pages! <br />
            <strong>To begin, select a page on the left (or add a new one 😊).</strong> <br />
            Anything that&apos;s related to the page as a whole (metadata, SEO settings, route, and much more!) will be
            available under &quot;Page&quot; on the right.
            <br />
            If you want to change the design of a Block (we call each element on a page a block), click on an element on
            the page and head to the &quot;block -{'>'} design&quot; section on the right. Want to change plugin
            settings or text of an element: &quot;block -{'>'} function&quot;!
          </p>
  ...
  const url = `${window.location.origin}/${activePage?.previewUrl ?? activePage?.route}`;
  ...
  return (
    <div className="h-full w-full relative">
      ...
      ></div>
      <BlockMenu />
      <iframe ref={pageFrameRef} className="h-full w-full scrollbar-md" src={url} />
    </div>
  );
};

export const PageFrame = () => {
  return <InnerPageFrame />;
};
