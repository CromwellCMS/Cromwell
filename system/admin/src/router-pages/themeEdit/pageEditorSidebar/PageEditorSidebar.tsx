  ...
  return (
    <>
        ...
        <div className="grid grid-cols-2">
          <span
            ...
            className={`border-b border-white p-2 w-full text-center ${
              view === 'design' ? 'text-black font-bold border-indigo-500 bg-white' : ''
            }`}
          >
            Design
          </span>
      ...
      <div className="w-full max-h-[calc(100%-70px)] overflow-y-scroll scrollbar-slim">
        {view === 'design' && <PageDesignEditor block={selectedEditableBlock.current} />}
        {view === 'plugin' && <PluginEditor block={selectedEditableBlock.current} setSidebarWidth={setSidebarWidth} />}
      </div>
    </>
  );
};
...
export const PageEditorSidebar = () => {
  ...
  const [blockView, setBlockView] = useState<'design' | 'plugin'>('design');
  const { editingPageConfig } = useThemeEditor();
  const { selectedEditableBlock, updateFramesPosition } = usePageBuilder();
  ...
  const resizePanel = useCallback(
    (event) => {
      ...
      const startingX = parseInt(`${Math.min(Math.max(190, start), 900)}`, 10);
      setTempW(startingX);
  ...
  useEffect(() => {
    ...
    const onUpdate = (event) => {
      if (tempW) {
        ...
        const nextVal = parseInt(`${Math.min(Math.max(190, start), 900)}`, 10);
        setResizeW(nextVal);
  ...
  return (
    <div
      ref={boundingRef}
      style={{ width: resizeW }}
      className="bg-white border-l h-screen border-gray-300   pb-14 w-[320px] relative"
    >
      <div onMouseDown={resizePanel} className="cursor-w-resize h-full top-0 left-0 w-[2px] absolute">
        <span className={`w-[1200px] h-full absolute right-0 top-0 ${isResizing ? '' : 'hidden'}`} />
      </div>
      ...
      <div className="bg-indigo-200 text-xs w-full px-4 text-gray-500 relative">
        <div className="grid grid-cols-3">
          <span
            ...
            className={`p-2 text-center ${blockSettings ? 'text-black font-bold bg-white rounded-t-md' : ''} ${
      ...
      <div className="h-full">
        {blockSettings && selectedEditableBlock.current && (
          <BlockSubViews view={blockView} setView={setBlockView} setSidebarWidth={setResizeW} />
        )}
        {!blockSettings && editingPageConfig && <PageSettings />}
