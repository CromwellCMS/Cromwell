      ...
      <Menu as="div" className="w-[calc(100%-24px)] relative inline-block">
        <div className="w-full relative inline-block">
          ...
          <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
            <Menu.Items
              className={`divide-y bg-white rounded-md divide-gray-100 shadow-lg ring-black mt-2 w-full  ${
                top ? 'origin-bottom-right -translate-y-full' : 'origin-top-right'
              } right-0 ring-1 ring-opacity-5 z-[100] absolute focus:outline-none`}
            >
                ...
                {options.map((opt, optIdx) => (
                  <Menu.Item key={optIdx}>
                    {({ active }) => (
                      ...
                      >
                        <span className={`block truncate ${active ? 'font-medium' : 'font-normal'}`}>{opt.title}</span>
                      </button>
                    )}
                  </Menu.Item>
                ))}
