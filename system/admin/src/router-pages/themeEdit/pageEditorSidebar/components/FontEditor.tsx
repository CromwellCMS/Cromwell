...
export const FontEditor = ({
  ...
  const styleWeight = weights.find((k) => k.value === styles?.fontWeight) ?? weights[3];

  const styleFontSize = fontSizes.find((k) => k.value.fontSize === styles?.fontSize) ?? { title: '-' };
  return (
    <>
      <p className="font-bold my-2 mt-0 text-xs uppercase">Text</p>
      <div className="grid gap-1 grid-cols-1">
        <ColorPickerField label="color" id="color" value={styles?.color || '#000000'} onChange={handleStyleChange} />
        {/* </div> */}
        ...
        <div className="font-bold mt-2 text-xs w-full">
          ...
          <div className="border rounded-md border-indigo-600 border-opacity-0 w-21 group hover:border-opacity-50 active:border-opacity-100 focus-within:border-opacity-100 hover:focus-within:border-opacity-100">
            <SelectableField
              value={styleWeight}
              onChange={(next) => handleStyleChange('fontWeight', next.value)}
              options={weights}
            />
          </div>
        </div>
        ...
        <div className="mt-0 grid gap-2 grid-cols-4">
          <button
            className={`${
              styles?.textAlign === 'left' ? 'bg-indigo-100' : ''
            } text-black text-center rounded-md hover:bg-indigo-600 hover:text-white`}
            onClick={() => handleStyleChange('textAlign', 'left')}
          >
            <Bars3BottomLeftIcon className="mx-auto h-4 w-4" />
          </button>
          <button
            className={`${
              styles?.textAlign === 'right' ? 'bg-indigo-100' : ''
            } text-black text-center rounded-md hover:bg-indigo-600 hover:text-white`}
            onClick={() => handleStyleChange('textAlign', 'right')}
          >
            <Bars3BottomRightIcon className="mx-auto h-4 w-4" />
          </button>
          <button
            className={`${
              styles?.textAlign === 'center' ? 'bg-indigo-100' : ''
            } text-black text-center rounded-md hover:bg-indigo-600 hover:text-white`}
          ...
          <button
            className={`${
              styles?.textAlign === 'justify' ? 'bg-indigo-100' : ''
            } text-black text-center rounded-md hover:bg-indigo-600 hover:text-white`}
