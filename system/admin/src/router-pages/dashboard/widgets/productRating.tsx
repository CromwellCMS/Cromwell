      ...
      <div className="relative">
        ...
        >
          {Array(maxValue)
            .fill(0)
            .map((star, idx) => (
              <EmptyStarIcon key={idx} className="h-6 text-orange-500 w-6 inline-block" />
            ))}
        </div>
        <div
          style={{
        ...
        >
          {Array(maxValue)
            .fill(0)
            .map((star, idx) => (
              <StarIcon key={idx} className="h-6 text-orange-500 w-6 inline" />
            ))}
        </div>
      </div>
    </div>
    ...
    <WidgetPanel isEditing={isEditing} id={id}>
      <div className="grid grid-cols-3 md:grid-cols-3">
        ...
        <div className={'col-span-2' + ' draggableCancel'}>
          <h3 className="block">Product rating</h3>
