  ...
  private setWidgetSize: WidgetTypes['Dashboard']['setSize'] = (pluginName, widgetLayouts) => {
    ...
    Object.keys(widgetLayouts).forEach((breakPoint) => {
      if (layout[breakPoint])
        layout[breakPoint] = layout[breakPoint].map((item) => {
        ...
        >
          <div key="productRating" className={styles.topStatItem}>
            <div className={styles.topStatContent}>
              <div
                className={styles.topStatIcon}
                style={{ backgroundImage: 'url(/admin/static/dashboard-rating.png)' }}
              ></div>
              <div className={styles.topStatCaption + ' draggableCancel'}>
                <h3 className={styles.topStatTitle}>Product Rating</h3>
                <Rating name="read-only" value={averageRating} className={styles.rating} precision={0.2} readOnly />
                <p className={styles.statTip}>
                  {averageRating.toFixed(2)}, based on {reviewsNumber} reviews
                </p>
              </div>
            </div>
          </div>
          <div key="salesValue" className={styles.topStatItem}>
            <div className={styles.topStatContent}>
              ...
              <div className={styles.topStatCaption + ' draggableCancel'}>
                ...
                <p className={styles.statBig} id="salesValue">
                  {this.state?.stats?.salesValue ?? 0}
                </p>
          ...
          <div key="pageViews" className={styles.topStatItem}>
            <div className={styles.topStatContent}>
              ...
              <div className={styles.topStatCaption + ' draggableCancel'}>
                ...
                <p className={styles.statBig} id="pageViews">
                  {this.state?.stats?.pageViews ?? 0}
                </p>
          ...
          <div key="ordersLastWeek" className={styles.chartBox}>
            <div className={styles.chartCaption}>
              ...
              <p className={styles.statBig + ' draggableCancel'} id="ordersTotalLastWeek">
                {this.state?.stats?.salesPerDay?.reduce<number>((prev, curr) => curr.orders + prev, 0) ?? 0}
              </p>
            </div>
            <ReactResizeDetector handleWidth handleHeight>
              {({ targetRef }) => {
                this.ordersChart?.resize?.();
                return <div id="ordersLastWeek" ref={targetRef as any} className={styles.chart}></div>;
              }}
            </ReactResizeDetector>
          </div>
          <div key="salesValueLastWeek" className={styles.chartBox}>
            <div className={styles.chartCaption}>
              ...
              <p className={styles.chartTotal + ' draggableCancel'} id="salesValueTotalLastWeek">
                {this.state?.stats?.salesPerDay?.reduce<number>((prev, curr) => curr.salesValue + prev, 0) ?? 0}
              </p>
            </div>
            <ReactResizeDetector handleWidth handleHeight>
              {({ targetRef }) => {
                this.salesValueChart?.resize?.();
                return <div id="salesValueLastWeek" ref={targetRef as any} className={styles.chart}></div>;
              }}
            </ReactResizeDetector>
          </div>
