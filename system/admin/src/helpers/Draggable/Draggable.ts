...
export class Draggable {
  ...
  private setupBlock = (block: HTMLElement) => {
    ...
    block.addEventListener('mousedown', (e) => {
      if (this.options.ignoreDraggableClass && block.classList.contains(this.options.ignoreDraggableClass)) return;
  ...
  private tryToInsert = throttle(100, (event: MouseEvent) => {
    ...
    const canInsert = this.canInsertBlock
      ? this.canInsertBlock(this.hoveredBlock, this.draggingBlock, afterElement)
      : true;
    ...
    if (canInsert) {
      ...
      try {
        ...
        if (!this.options.disableInsert) this.onBlockInserted?.(this.hoveredBlock, this.draggingBlock, afterElement);
      } catch (e) {
        console.error(e);
      }
    }
  ...
  private getDragAfterElement(container: HTMLElement, clientY: number): Element | null {
    const draggableElements = Array.from(container.children).filter(
      (child) =>
        child.classList.contains(Draggable.draggableBlockClass) && !child.classList.contains(Draggable.draggingClass),
    );
    ...
    if (this.options.disableInsert) {
      ...
      if (this.lastInsertionData)
        this.onBlockInserted?.(
          this.lastInsertionData.container,
          this.lastInsertionData.draggingBlock,
          this.lastInsertionData.afterElement,
        );
  ...
  private onPageBodyClick = (event: MouseEvent) => {
    if ((event as any).hitBlock) return;
    if (!this.options.disableClickAwayDeselect) this.deselectCurrentBlock();
  };
    ...
    if (block) {
      ...
      const styleParent = (block: HTMLElement) => {
        ...
        if (parent) {
          ...
          if (this.draggableBlocks.includes(parent) && this.options.applyZIndex) parent.style.zIndex = '1005';
          styleParent(parent);
        }
      };
      styleParent(block);
