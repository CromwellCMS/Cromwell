...
export class FontSize implements InlineTool {
  ...
  private tag: string;
  private icon =
    '<svg viewBox="0 0 24 24" width="20" height="18"><path d="M9 4v3h5v12h3V7h5V4H9zm-6 8h3v7h3v-7h3V9H3v3z"></path></svg>';
  ...
  showActions() {
    ...
    layoutActions.renderPortal(
      'fontSizeTool',
      <InputSlider initialVal={parseFloat(initialSize) ?? 16} onChange={this.handleSizeChange} />,
      this.actions,
    );
  }
  ...
  wrap() {
    ...
    if (firstParent && firstParent.tagName === 'SPAN' && firstParent.innerText === range.toString()) {
      ...
      if (
        this.currentSpan?.parentElement?.classList.contains('fontSize') &&
    ...
    if (this.currentSpan) this.api.selection.expandToTag(this.currentSpan);
  }
