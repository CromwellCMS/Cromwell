...
> = (props) => {
  ...
  const [isChangingTheme, setChangingTheme] =
    useState(false);
  ...
  const fetchConfigAndPackages = useCallback(async () => {
    try {
      ...
      infos?.sort((a) =>
        updatedConfig && a.name === updatedConfig.themeName
          ? -1
          : 1,
      );
  ...
  const getThemeList = async () => {
    ...
    if (graphQLClient) {
      try {
        const themeEntities: TThemeEntity[] =
  ...
  const getThemeUpdates = async () => {
    for (const theme of packages) {
      ...
      try {
        const update =
          await getRestApiClient().getThemeUpdate(
            theme.name,
          );
        if (update) {
      ...
      <div className="flex flex-row gap-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              {packages.map((info) => {
                const isActive = Boolean(
                  cmsConfig &&
                    cmsConfig.themeName === info.name,
                );
                const entity = installedThemes?.find(
                  (ent) => ent.name === info.name,
                );
                const isInstalled =
                  entity?.isInstalled ?? false;
                const availableUpdate =
                  themeUpdates[info.name];
        ...
        <div className="hidden lg:inline-block h-auto max-h-[24rem] xl:max-h-[22rem] bg-white dark:bg-gray-900 max-w-xs xl:max-w-md rounded-xl shadow-md shadow-indigo-300 p-4 mt-2">
          ...
          <p className="py-2 text-sm">
            ...
            frontend and host it wherever you like. But in
            this scenario, you need to manage and deploy
            this frontend by yourself. To simply the
            workflow Cromwell CMS has its theming engine.
            Users can easily install Themes from the
            official market right in their Admin panel GUI,
            make active, delete them, change layout in the
...
> {
  ...
  private handleSetActiveTheme = async (
    info: TPackageCromwellConfig,
  ) => {
    const client = getRestApiClient();
  ...
  private handleDelete = async (
    ...
    let success;
    try {
      success = await getRestApiClient().deleteTheme(
        theme.name,
      );
    } catch (error) {
      console.error(error);
...
const UpdateModalContent = (props: {
  ...
  const isUnderUpdate =
    entity?.name && props.underUpdate[entity.name];
