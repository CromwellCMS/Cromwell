...
import { PencilIcon, TrashIcon, PlusIcon, CogIcon } from '@heroicons/react/outline';
...
export function VariantsTab({ product, setProdData, forceUpdate, usedVariantAttributes, attributes,
    ...
    <Box id="product-variants-tab" sx={{ width: '100%' }}>
          ...
          <Box key={variant.id} id={`variant-${variant.id}`} className={styles.paper} sx={{ mb: 4, width: '100%' }}>
            <Box sx={{
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', px: 1 }} flexWrap="wrap" justifyContent="space-between">
                {attributes?.filter(attr => usedVariantAttributes.includes(attr.key))?.map(attr => (
                  <SelectInput
                    label={attr.key}
                    key={attr.key}
      ...
      <Box sx={{ width: '100%', my: 4, display: 'flex' }}>
        <Box sx={{ mx: 'auto', display: 'flex' }}>
          <TextButton style={{ marginRight: '15px' }}
            onClick={handleAddProductVariant}><PlusIcon className="w-5 h-5 mr-1" />Add product variant</TextButton>
          <TextButton onClick={handleEditAttributes}><CogIcon className="w-5 h-5 mr-1" />Edit attributes</TextButton>
          <Popover open={isEditingAttributes} anchorEl={popperAnchorEl}
              ...
              {attributes.map(attr => {
                return (
                  <div key={attr.key} style={{
                    ...
                    width: '100%', minWidth: '20px', padding: '10px 15px'
                  }}>
                    <div className={commonStyles.center}>
                      <SwitchInput value={!!usedVariantAttributes.includes(attr.key)}
