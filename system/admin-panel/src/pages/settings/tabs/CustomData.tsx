...
export default function CustomData(props: any) {
  ...
  const saveCustomEntity = () => {
    ...
    if (settings?.customEntities?.find((ent) => ent.entityType === entityToEdit.entityType)) {
      changeSettings(
        'customEntities',
        [...settings.customEntities].map((ent) => {
  ...
  const deleteCustomEntity = (entityType: string) => {
    if (settings?.customEntities) {
      changeSettings(
        'customEntities',
        [...settings.customEntities].filter((ent) => {
  ...
  return (
    <Grid container spacing={3}>
      ...
      <Grid item xs={12}>
        <Button onClick={handleAddEntity} variant="outlined">
          Add entity
        </Button>
      </Grid>
      <Modal open={!!entityToEdit} onClose={handleCloseAddEntity} className={styles.entityToEdit}>
        <TextField
          label="Type"
        ...
        <div style={{ display: 'flex' }}>
          ...
          <Button variant="contained" onClick={saveCustomEntity}>
            Apply
          </Button>
        </div>
      </Modal>
...
const CustomFieldSettings = (props: { data: TCustomFieldSettingsData }) => {
  ...
  const deleteField = () => {
    changeSettings(
      'customFields',
      (settings?.customFields ?? []).filter((field) => field.id !== fieldData.id),
    );
