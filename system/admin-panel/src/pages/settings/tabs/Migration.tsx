...
export default function Migration() {
  ...
  const exportDB = async () => {
    setIsExporting(true);
    try {
      await getRestApiClient().exportDB(exportOptions.filter((opt) => opt.checked).map((opt) => opt.key));
    } catch (e) {
      console.error(e);
    }
    setIsExporting(false);
  ...
  return (
    <Grid container spacing={3}>
      <Grid item xs={12}>
        ...
        <FormControlLabel
          control={<Checkbox checked={removeSurplus} onChange={() => setRemoveSurplus(!removeSurplus)} />}
