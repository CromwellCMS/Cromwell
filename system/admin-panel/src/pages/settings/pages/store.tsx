...
export const StoreSettingsPage = () => {
  const { adminSettings, saveStoreSettings } =
    useAdminSettings();
  ...
  const shippingDirty =
    methods.formState.dirtyFields.defaultShippingPrice;
  const currencyDirty = Object.keys(
    methods.formState.dirtyFields.currencies || {},
  )
    .map((o) => {
      const fld =
        methods.formState.dirtyFields.currencies[o];
      return (
        fld?.id ||
        fld?.tag ||
        fld.title ||
      ...
      <form
        ...
        <div className="flex flex-col mt-6 gap-6 relative lg:flex-row">
          <div className="flex-shrink max-h-min my-4 lg:max-w-[13rem] top-16 self-start lg:order-2 lg:sticky">
            ...
            <p>
              Add currencies, exchange rates and set your
              primary currency. To change a currency to
              primary, drag it from{" "}
              <GrabIcon className="h-4 mx-1 w-4 inline-block" />{" "}
