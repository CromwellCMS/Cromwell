...
const ControlledPermissionOption = ({
  name = "",
  data = null,
}: {
  name: string;
...
const ControlledPermissionCategory = ({
  ...
  const value = watch(fields).reduce(
    (a, v) => a && v,
    true,
  );
...
export const CustomRoleSettingsPage = () => {
  ...
  const orderedPermissions = useMemo(() => {
    return {
      ...
      plugins: permissions.filter((p) =>
        p.name.includes("_plugin"),
      ),
      themes: permissions.filter((p) =>
        p.name.includes("_theme"),
      ),
      posts: permissions.filter(
        (p) =>
          p.name.includes("_post") &&
          !p.name.includes("_post_comment"),
      ),
      comments: permissions.filter((p) =>
        p.name.includes("_post_comment"),
      ),
      tags: permissions.filter((p) =>
      ...
      categories: permissions.filter((p) =>
        p.name.includes("_product_categ"),
      ),
      attributes: permissions.filter((p) =>
        p.name.includes("_attribut"),
      ),
      orders: permissions.filter((p) =>
        p.name.includes("_order"),
      ),
      reviews: permissions.filter((p) =>
        p.name.includes("_review"),
      ),
      users: permissions.filter((p) =>
      ...
      coupons: permissions.filter((p) =>
        p.name.includes("_coupon"),
      ),
      cms: permissions.filter((p) =>
        p.name.includes("_cms"),
      ),
      publicDir: permissions.filter((p) =>
        p.name.includes("_public_dir"),
      ),
      files: permissions.filter((p) =>
        p.name.includes("_file"),
      ),
      system: permissions.filter((p) =>
        ...
        <div className="flex flex-col mt-8 z-4 gap-6 relative lg:flex-row">
          <div className="max-h-min my-4 top-16 self-start lg:order-2 lg:max-w-[13rem] lg:sticky">
            ...
            <p className="text-red-600">
              Warning: Wrong configuration may expose
              features to roles!
            </p>
            <p
              className={`${
          ...
          <div
            ...
            <div className="flex flex-wrap">
              <ControlledPermissionCategory
                ...
                fields={[
                  ...orderedPermissions.system,
                  ...orderedPermissions.cms,
                ].map((o) => `permissions.${o.name}`)}
              />
              {orderedPermissions.system.map((opt) => (
                <div
                  key={opt.name}
                  className="max-h-[200px] p-2 basis-full lg:basis-1/2">
                  <ControlledPermissionOption
                    key={opt.name}
                    name={`permissions.${opt.name}`}
                    data={opt}
                  />
                </div>
              ))}
              ...
              <ControlledPermissionCategory
                ...
                fields={orderedPermissions.products.map(
              ...
              <ControlledPermissionCategory
                ...
                fields={orderedPermissions.orders.map(
                  (o) => `permissions.${o.name}`,
                )}
              />
              ...
              <ControlledPermissionCategory
                ...
                fields={orderedPermissions.categories.map(
                  (o) => `permissions.${o.name}`,
                )}
              />
              ...
              <ControlledPermissionCategory
                ...
                fields={orderedPermissions.attributes.map(
                  (o) => `permissions.${o.name}`,
                )}
              />
              ...
              <ControlledPermissionCategory
                ...
                fields={orderedPermissions.reviews.map(
                  (o) => `permissions.${o.name}`,
                )}
              />
              ...
              <ControlledPermissionCategory
                ...
                fields={orderedPermissions.coupons.map(
                  (o) => `permissions.${o.name}`,
                )}
              />
              ...
              <ControlledPermissionCategory
                ...
                fields={orderedPermissions.posts.map(
                  (o) => `permissions.${o.name}`,
                )}
              />
              ...
              <ControlledPermissionCategory
                ...
                fields={orderedPermissions.tags.map(
                  (o) => `permissions.${o.name}`,
                )}
              />
              ...
              <ControlledPermissionCategory
                ...
                fields={orderedPermissions.comments.map(
                  (o) => `permissions.${o.name}`,
                )}
              />
              ...
              <ControlledPermissionCategory
                ...
                fields={orderedPermissions.users.map(
                  (o) => `permissions.${o.name}`,
                )}
              />
              ...
              <ControlledPermissionCategory
                ...
                fields={orderedPermissions.roles.map(
                  (o) => `permissions.${o.name}`,
                )}
              />
              ...
              <ControlledPermissionCategory
                ...
                fields={[
                  ...orderedPermissions.publicDir,
                  ...orderedPermissions.files,
                ].map((o) => `permissions.${o.name}`)}
              />
              ...
              <ControlledPermissionCategory
                ...
                fields={orderedPermissions.entities.map(
                  (o) => `permissions.${o.name}`,
                )}
              />
              ...
              <ControlledPermissionCategory
                ...
                fields={orderedPermissions.db.map(
                  (o) => `permissions.${o.name}`,
                )}
              />
              ...
              <hr className="my-4" />
              <div className="-mb-2 ml-4 block basis-full">
                <h2 className="font-bold mt-8 basis-full">
                  Third Party Permissions
                </h2>
                <p className="text-xs text-gray-700">
                  All permissions that are from third party
                  providers (plugins)
                </p>
              </div>
