...
const inferValue = (v: string) =>
  selectOptions.find((k) => k.value === v);

export function EntityFieldItem(props: {
  idx?: number;
  id?: any;
  field?: FieldArrayWithId<
    CustomEntityFormType,
    "customFields",
    "id"
  >;
  ...
  const { register, watch, getValues, setValue } =
    useFormContext<CustomEntityFormType>();
  ...
  const showOptions =
    watch(
      `customFields.${idx}.fieldType`,
      getValues(`customFields.${idx}.fieldType`),
    ) === "Select";
  ...
  return (
    <div
      ...
      <div className="flex flex-row gap-1 justify-between">
              ...
              <>
                <div className="flex gap-2 justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      optionMethods.append("", { shouldFocus: true })
                    }
                ...
                <div className="rounded-md flex flex-col bg-gray-200 shadow-inner min-h-[25px] p-1 gap-1">
                  {
                    optionMethods.fields.map((option, oIdx) => (
                      <div key={oIdx} className="flex flex-row w-full gap-1">
                        ...
                        />
                        <button onClick={() => optionMethods.remove(oIdx)} type="button" className="mb-2 text-gray-600 self-end hover:text-red-500">
        ...
        <div
          className="rounded-md flex h-full grow touch-none hover:bg-indigo-100"
          {...listeners}
          {...attributes}>
...
export const DraggableEntityFields = ({
  entityType,
}: {
  entityType: string;
}) => {
  ...
  return (
    <div className="">
      <div className="flex flex-row mb-2 gap-2 justify-between">
        <p className="font-bold my-1 self-center">
          Entity Fields
        </p>
        <button
          type="button"
