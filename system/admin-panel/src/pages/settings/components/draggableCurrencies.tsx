...
type Currency = ArrayElement<
  TAdminCmsSettingsType["currencies"]
>;
type FormType = Pick<
  TAdminCmsSettingsType,
  "defaultShippingPrice" | "currencies"
>;
...
export function CurrencyItem(props: {
  ...
  const symbol = watch(
    `currencies.${idx}.symbol`,
    getValues(`currencies.${idx}.symbol`),
  );
  ...
  return (
    <div
      ...
      <div className="mb-2 block">
        {primary && (
          <span className="rounded-xl bg-indigo-700 text-white text-xs text-center py-1 px-3">
            primary
          </span>
        )}
      </div>
      <div className="flex flex-row gap-1 justify-between">
        <div className="font-bold mr-2 text-2xl text-indigo-600">
          {symbol}
        </div>
        ...
        <div
          className="rounded-md flex h-full grow hover:bg-indigo-100"
          {...listeners}
          {...attributes}>
