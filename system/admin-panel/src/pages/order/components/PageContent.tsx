...
export function PageContent(props: TFieldsComponentProps<TOrder> & {
      ...
      <div className={styles.content}>
        <div className={styles.fields}>
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6}>
              <TextInput label="Shipping method"
                disabled
            ...
            <Grid item xs={12} sm={6}>
              <TextInput label="Shipping price"
        ...
        <div className={styles.sums}>
          {data && !cartLoading && (
            <>
              <p>Cart total: <b>{cstore.getPriceWithCurrency(cartUpdatedRef.current ?
                updatedCartTotal : data.cartTotalPrice)}</b></p>
              <p>Shipping: <b>{cstore.getPriceWithCurrency(data.shippingPrice ?? 0)}</b></p>
              <p>Order total: <b>{cstore.getPriceWithCurrency(cartUpdatedRef.current ?
        ...
        <div className={styles.cart}>
          {!cartLoading && cart?.map((it, i) => {
            const product = Object.assign({}, cartProducts.find(p => p.id === it?.product?.id), it.product);
                ...
                <Grid key={i} className={styles.cartItem} container>
                  <Grid item xs={1} className={styles.itemBlock}>
                    <Link to={{
                      ...
                      state: { prevRoute: history.location.pathname + window.location.search }
                    }}>
                  ...
                  <Grid item xs={4}>
                    <div className={styles.itemBlock}>
                      <Link className={styles.productName}
                        to={{
                          pathname: productLink,
                          state: { prevRoute: history.location.pathname + window.location.search }
                        }}
