...
import { CodeIcon, HashtagIcon, InformationCircleIcon, LinkIcon } from '@heroicons/react/outline';
...
import { askConfirmation, toast } from '../../../../exports';
  ...
  return (
    <div className="mt-6 px-3">
      ...
      <TextInput
        ...
        prefixElement={<LinkIcon className="h-4 w-4 inline" />}
        name="route"
        value={config?.route || ''}
        label="route"
      ...
      />
      <TextInput
        prefixElement={<InformationCircleIcon className="h-4 w-4 inline" />}
        name="name"
        value={config?.name || ''}
        label="name (internal)"
      ...
      />
      <TextInput
        prefixElement={<div className="h-4 text-xs pl-1 w-4 inline">T</div>}
        name="title"
        value={config?.title || ''}
        label="meta title"
      ...
      <TextAreaInput
        rows={4}
        prefixElement={<HashtagIcon className="h-4 w-4 inline" />}
        name="description"
      ...
      <span className="font-bold mt-3 text-xs block">Head HTML</span>
      <CodeEditor
        className="bg-white rounded-lg min-h-[150px]"
        language="html"
        value={config?.headHtml}
        onChange={(e) => onInputChange('headHtml', e.target.value)}
      />
      ...
      <span className="font-bold mt-3 text-xs block">Footer HTML</span>
      <CodeEditor
        className="bg-white rounded-lg min-h-[150px]"
        language="html"
        value={config?.footerHtml}
        onChange={(e) => onInputChange('footerHtml', e.target.value)}
      />
    </div>
  );
};
...
export const PageSettings = () => {
  ...
  const [config, setTmpConfig] = useState(editingPageConfig);
  const genericPages = themeConfig?.genericPages ?? [{ route: 'pages/[slug]', name: 'default' }];

  const pageLayout = useRef(config?.layoutRoute ?? genericPages[0].route);
  ...
  return (
    <div className="w-full scrollbar-slim overflow-x-hidden overflow-y-auto">
      <PageMetaSettings onInputChange={handleChange} config={config} />
      <hr className="mt-12" />
