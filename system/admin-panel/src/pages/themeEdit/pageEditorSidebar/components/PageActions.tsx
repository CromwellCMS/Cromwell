...
export const PageActions = () => {
  ...
  const [previewUrl, setPreviewUrl] = useState(
    (config?.previewUrl ?? config?.route) || ""
  );
  ...
  const handleChange = (
    ...
    if (config?.isVirtual && prop === "route") {
      if (!val) val = "";
      const prefix = pageLayout.current.route.replace(
        "[slug]",
        "",
      );
