...
export const ImageBlockEditor = ({
  ...
  return (
    <div className="text-xs p-2">
      ...
      <TextInput prefixElement="🔗" label="Link to" value={imageData?.link} onChange={(e) => handleTextInput('link')(e.target.value)} />
      <TextInput prefixElement="📝" label="Description (alt)" value={imageData?.alt} onChange={(e) => handleTextInput('alt')(e.target.value)} />
      <label className="font-bold mt-3 text-xs block">
        width
      <StyleNumberField dataType="px" value={imageData?.width} label="W" min={0} name="" keyName="width" handleStyleChange={(keyname, val, unit) => {
        if ((val || val !== "") && (!unit || unit === "px"))  return;
        handleTextInput('width')(`${val}${unit && unit !== "" ? unit : ""}`)
      }} />
      </label>
      <label className="font-bold mt-3 text-xs block">
        height
      <StyleNumberField dataType="px" value={imageData?.height} label="H" min={0} name="" keyName="height" handleStyleChange={(keyname, val, unit) => {
        if ((val || val !== "") && (!unit || unit === "px"))  return;
        handleTextInput('height')(`${val}${unit && unit !== "" ? unit : ""}`)
      }} />
      </label>

      <label className="font-bold mt-3 text-xs block">
        image fit
        <SelectableField top value={objectFit} options={fitOptions} onChange={(next) => {
