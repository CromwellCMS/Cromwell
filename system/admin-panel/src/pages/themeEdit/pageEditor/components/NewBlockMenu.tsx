...
export const NewBlockMenu = ({ onAddBlock = () => {}, position = "bottom" }: { onAddBlock?: any, position?: "top"|"bottom" }) => {
  ...
  const allPlugins = [...defaultOptions, ...(plugins as any[]).map(p => {
    p.type = "plugin";
    p.plugin = {
      pluginName: p.name,
    }
  ...
  return (
    <div ref={element} className={`transform ${position === "bottom" ? "-bottom-7" : "-top-8"} left-1/2 -translate-x-1/2 absolute`}>
      <Listbox value={null} onChange={(newBlock) => {
        onAddBlock(newBlock, position);
      }}>
        <div className="mt-1 relative">
          <Listbox.Button className={`border ${position === "bottom" ? "rounded-b-lg" : "rounded-t-lg"} font-bold bg-indigo-600 border-indigo-800 h-7 shadow-lg text-center text-white p-1 shadow-indigo-400 w-14 pointer-events-auto uppercase hover:bg-indigo-500`}>
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <Listbox.Options className={`${showBottom && position === "bottom" ? "" : "top-[-240px]"} ${showBottomInverse && position === "top" ? "top-8" : "top-[-240px]"} bg-white rounded-md shadow-md ring-black mt-1 text-base max-h-60 transform pb-1 shadow-indigo-400 ring-1 ring-opacity-5 w-72 z-[100] -translate-x-1/2 left-1/2 scrollbar-slim absolute overflow-auto pointer-events-auto sm:text-sm focus:outline-none`}>
              <input value={searchValue} onChange={(e) => setSearchValue(e.target.value || "")} className="border mx-auto border-gray-100 shadow-sm mb-2 w-full py-1 px-2 top-0 z-[20] sticky" placeholder="Search..." />
              {allPlugins.filter(k => {
