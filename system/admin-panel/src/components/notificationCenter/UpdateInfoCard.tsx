      ...
      {updateInfo.onlyManualUpdate && (
        <div className={styles.warning}>
          ...
          <p style={{ margin: '0 0 0 5px' }}>
            Cannot launch automatic update. Please update using
            <span className={styles.command}>yarn upgrade @cromwell/cms@{updateInfo.packageVersion} --exact</span>
            command and restart CMS
          </p>
        </div>
      )}
      <Collapse in={!changelogCollapsed} timeout="auto" unmountOnExit>
        <div className={styles.changelogList} dangerouslySetInnerHTML={{ __html: updateInfo.changelog }}></div>
      </Collapse>
      <Button disabled={updateInfo.onlyManualUpdate} color="primary" variant="contained" onClick={props.onStartUpdate}>
        Update
      </Button>
    </Grid>
  );
