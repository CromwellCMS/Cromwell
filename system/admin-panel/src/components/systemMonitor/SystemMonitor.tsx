    ...
    private onOpen = async () => {
        ...
        if (this.props.open) {
            ...
            this.cpuUsageChart.setOption(
                getCpuUsageOption(this.echarts, this.state?.system?.cpuUsage?.previousLoads ?? []));

            this.ramUsageChart.setOption(
                getPieOption('RAM', this.state?.system?.memoryUsage?.available ?? 0,
                    this.state?.system?.memoryUsage?.used ?? 0));

            this.diskUsageChart.setOption(
                getPieOption('Disk', this.state?.system?.diskUsage?.available ?? 0,
                    this.state?.system?.diskUsage?.used ?? 0));
        }
    }
                ...
                <div className={styles.SystemMonitor}>
                    ...
                    {this.state?.system && (
                        <div className={styles.specsList}>
                            ...
                            <p><b>RAM:</b> {prettyBytes(this.state?.system?.memoryUsage?.total ?? 0)}</p>
                            <p><b>Disk:</b> {prettyBytes(this.state?.system?.diskUsage?.size ?? 0)}</p>
                        </div>
                    )}
