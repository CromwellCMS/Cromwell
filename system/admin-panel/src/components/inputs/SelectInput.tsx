...
export const SelectInput = <T extends unknown>({
  ...
  const displayValue = getDisplayValue ? getDisplayValue(value) : typeof value === 'object' && value ? (value as any).label : value;
      ...
      <Listbox disabled={disabled} value={getValue ? getValue(value) : value} onChange={onChange}>
        ...
        <div className="relative" >
          <Listbox.Button disabled={disabled}
            className={`bg-white disabled:bg-gray-100 border-2 ${error ? "border-red-500" : ""} rounded-md cursor-default shadow-md shadow-indigo-50 text-left ${error ?
              "text-red-500" : ''} w-full py-2 pr-10 pl-3 relative sm:text-sm focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-white focus-visible:ring-2 focus-visible:ring-opacity-75 focus-visible:ring-offset-indigo-300 focus-visible:ring-offset-2 ${className ?? ""}`}>
          ...
          >
            <Listbox.Options className="bg-white rounded-md shadow-lg ring-black mt-1 text-xs w-full max-h-60 py-1 ring-1 ring-opacity-5 z-[100] absolute overflow-auto sm:text-sm focus:outline-none">
              {(options as any[]).map((option, optionIdx) => {
                const label = getDisplayValue ? getDisplayValue(option) : typeof option === 'object' && option ? option.label : option;
                const value = getValue ? (getValue(option)) : typeof option === 'object' && option ? option.value : option;
                return (
                  <Listbox.Option
                    key={optionIdx}
                    className={({ active }) =>
