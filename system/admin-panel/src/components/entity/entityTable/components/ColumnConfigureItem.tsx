...
export const ColumnConfigureItem = (props: { data: TColumnConfigureItemData }) => {
  const data = props.data;
  const [visible, setVisible] = useState(data.sortedColumns[data.column.name]?.visible ?? data.column.visible);
  const toggleVisibility = () => {
    setVisible(!visible);
    if (!data.sortedColumns[data.column.name])
      data.sortedColumns[data.column.name] = {
  ...
  return (
    ...
    >
      <p className={styles.ellipsis} style={{ minWidth: '20px' }}>
        {props.data.column.label}
      </p>
      <div className={commonStyles.center}>
        <SwitchInput value={visible} onChange={toggleVisibility} />
      </div>
    </div>
  );
};
