...
module.exports = class coupons1638975069604 {
  ...
  async up(queryRunner) {
    await queryRunner.query(
      `CREATE TABLE "crw_coupon_meta" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "key" varchar(255), "value" text, "shortValue" varchar(255), "entityId" integer NOT NULL)`,
    );
    ...
    await queryRunner.query(
      `CREATE TABLE "crw_order_coupons_coupon" ("orderId" integer NOT NULL, "couponId" integer NOT NULL, PRIMARY KEY ("orderId", "couponId"))`,
    );
    ...
    await queryRunner.query(
      `CREATE TABLE "temporary_crw_coupon_meta" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "key" varchar(255), "value" text, "shortValue" varchar(255), "entityId" integer NOT NULL, CONSTRAINT "FK_ae33d7acbc699d8192c2eea22cf" FOREIGN KEY ("entityId") REFERENCES "crw_coupon" ("id") ON DELETE CASCADE ON UPDATE NO ACTION)`,
    );
    await queryRunner.query(
      `INSERT INTO "temporary_crw_coupon_meta"("id", "key", "value", "shortValue", "entityId") SELECT "id", "key", "value", "shortValue", "entityId" FROM "crw_coupon_meta"`,
    );
  ...
  async down(queryRunner) {
    ...
    await queryRunner.query(
      `INSERT INTO "crw_coupon_meta"("id", "key", "value", "shortValue", "entityId") SELECT "id", "key", "value", "shortValue", "entityId" FROM "temporary_crw_coupon_meta"`,
    );
