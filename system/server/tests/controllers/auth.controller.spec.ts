...
describe('auth Controller', () => {
  ...
  it(`/POST login`, () => {
    return request(server).post('/v1/auth/login').send({ email: 'test@test.test', password: 'test' }).expect(200);
  });

  it(`/POST log-out`, () => {
    return request(server).post('/v1/auth/log-out').expect(200);
  });
  ...
  it(`/POST reset-password`, async () => {
    ...
    return request(server).post('/v1/auth/login').send({ email: 'test@test.test', password: 'test2' }).expect(200);
  });
