...
export class MigrationService {
  ...
  private async exportPosts(workbook: any) {
    ...
    const postSheet: Record<keyof TPostInput, any>[] = await Promise.all(
      posts.map(async (ent) => ({
        id: ent.id,
        slug: ent.slug,
  ...
  private async exportTags(workbook: any) {
    ...
    const tagsSheet: Record<keyof TTagInput, any>[] = await Promise.all(
      tags.map(async (ent) => ({
        id: ent.id,
        slug: ent.slug,
        pageTitle: ent.pageTitle,
  ...
  private async exportProducts(workbook: any) {
    ...
    const productsSheet: Record<keyof TProductInput, any>[] = await Promise.all(
      products.map(async (ent) => ({
        id: ent.id,
        slug: ent.slug,
        pageTitle: ent.pageTitle,
        pageDescription: ent.pageDescription,
        ...
        description: ent.description,
        descriptionDelta: ent.descriptionDelta,
        attributes: this.stringifyValue(
          getCustomRepository(ProductRepository).attributeRecordsToProductAttributeInstances(ent.attributeValues),
        ),
        customMeta: this.stringifyValue(
          await entityMetaRepository.getEntityMetaByKeys(EDBEntity.Product, ent.id, metaKeys),
        ),
        views: undefined,
        variants: undefined,
      })),
  ...
  private async exportProductVariants(workbook: any) {
    ...
    const sheet: Record<keyof TProductVariantInput, any>[] = await Promise.all(
      variants.map(async (ent) => ({
        id: ent.id,
        ...
        customMeta: this.stringifyValue(
          await entityMetaRepository.getEntityMetaByKeys(EDBEntity.ProductVariant, ent.id, metaKeys),
        ),
        views: undefined,
      })),
    );
  ...
  private async exportCategories(workbook: any) {
    ...
    const categoriesSheet: Record<keyof TProductCategoryInput, any>[] = await Promise.all(
      categories.map(async (ent) => ({
        id: ent.id,
        slug: ent.slug,
        pageTitle: ent.pageTitle,
        pageDescription: ent.pageDescription,
        ...
        customMeta: this.stringifyValue(
          await entityMetaRepository.getEntityMetaByKeys(EDBEntity.ProductCategory, ent.id, metaKeys),
        ),
        views: undefined,
  ...
  private async exportAttributes(workbook: any) {
    ...
    const attributesSheet: Record<keyof TAttributeInput, any>[] = await Promise.all(
      attributes.map(async (ent) => ({
        id: ent.id,
        slug: ent.slug,
        title: ent.title,
        pageTitle: ent.pageTitle,
        ...
        customMeta: this.stringifyValue(
          await entityMetaRepository.getEntityMetaByKeys(EDBEntity.Attribute, ent.id, metaKeys),
        ),
        views: undefined,
      })),
    );
  ...
  private async exportReviews(workbook: any) {
    ...
    const reviewsSheet: Record<keyof TProductReviewInput, any>[] = await Promise.all(
      reviews.map(async (ent) => ({
        id: ent.id,
        slug: ent.slug,
        pageTitle: ent.pageTitle,
        pageDescription: ent.pageDescription,
  ...
  private async exportCoupons(workbook: any) {
    ...
    const dataSheet: Record<keyof TCouponInput, any>[] = await Promise.all(
      reviews.map(async (ent) => ({
        id: ent.id,
        slug: ent.slug,
        pageTitle: ent.pageTitle,
  ...
  private async exportUsers(workbook: any) {
    ...
    const usersSheet: Record<keyof TUpdateUser, any>[] = await Promise.all(
      users.map(async (ent) => ({
        id: ent.id,
        slug: ent.slug,
        pageTitle: ent.pageTitle,
  ...
  private async exportRoles(workbook: any) {
    ...
    const dataSheet: Record<keyof TRole, any>[] = await Promise.all(
      roles.map(async (ent) => ({
        id: ent.id,
        slug: ent.slug,
        pageTitle: ent.pageTitle,
  ...
  private async exportCustomEntities(workbook: any) {
    ...
    const customEntitiesSheet: Record<keyof TCustomEntityInput, any>[] = await Promise.all(
      customEntities.map(async (ent) => ({
        id: ent.id,
        slug: ent.slug,
        pageTitle: ent.pageTitle,
        pageDescription: ent.pageDescription,
  ...
  private parseBaseInput<TEntity extends TBaseEntityRecord>(
    input: TEntityRecord<TEntity>,
  ): Required<TBaseEntityRecord> {
    return {
  ...
  private async importCategories(options: TImportOptions) {
    await this.importBase<TProductCategoryInput>({
      ...
      update: async (input) =>
        input.id && getCustomRepository(ProductCategoryRepository).updateProductCategory(input.id, input),
      ...
      modifyInputs: (inputs: (TProductCategoryInput & { id?: number | null })[]) => {
        ...
        const collectParentsChain = (
          input: TProductCategoryInput,
          parents: TProductCategoryInput[],
        ): TProductCategoryInput[] => {
  ...
  private async importProductVariants(options: TImportOptions) {
    await this.importBase<TProductVariantInput>({
      ...
      update: async (input) =>
        input.id && getCustomRepository(ProductVariantRepository).updateProductVariant(input.id, input),
  ...
  private async importReviews(options: TImportOptions) {
    await this.importBase<TProductReviewInput>({
      ...
      update: async (input) =>
        input.id && getCustomRepository(ProductReviewRepository).updateProductReview(input.id, input),
        ...
        userRepo.createUser(
          {
            ...input,
            password:
              input.password || (await userRepo.hashPassword(cryptoRandomString({ length: 14, type: 'url-safe' }))),
          },
          input.id,
          'hash',
        ),
  ...
  private async importCustomEntities(options: TImportOptions) {
    await this.importBase<TCustomEntityInput>({
      ...
      update: async (input) =>
        input.id && getCustomRepository(CustomEntityRepository).updateCustomEntity(input.id, input),
  ...
  private async importCmsSettings(options: TImportOptions) {
    const inputs = this.readSheet<Record<keyof CmsEntity, string | null | undefined | boolean>>(
      options.workbook,
      ESheetNames.CMS,
    );
    if (!inputs?.[0]) return;
