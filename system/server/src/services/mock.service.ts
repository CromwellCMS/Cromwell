...
export class MockService {
  ...
  private attributesMock: TAttributeInput[] = [
    ...
    },
    {
      ...
      title: 'Color',
      values: [
        { value: 'Orange', icon: '/plugins/@cromwell/plugin-product-filter/color_orange.png' },
  ...
  public async mockProducts(amount?: number): Promise<boolean> {
    const images = [
      '/themes/@cromwell/theme-store/product1.jpg',
      '/themes/@cromwell/theme-store/product2.jpg',
      '/themes/@cromwell/theme-store/product3.jpg',
    ];
    const imagesNum = 6;
    ...
    for (let i = 0; i < 14; i++) {
      ...
      for (let j = 0; j < Math.ceil((Math.random() * (amount ?? 56)) / 14); j++) {
        ...
        const createOrder = async () => {
          const cart: TStoreListItem[] = this.getRandomElementsFromArray(products.elements!, 3).map(
