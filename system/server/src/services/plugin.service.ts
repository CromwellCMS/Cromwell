...
export class PluginService {
  ...
  public async getPluginBundle(
    ...
    if (!pluginDir) {
      logger.error(
        'Failed to resolve plugin directory of: ' +
          pluginName +
          ". Probably plugin was used in module config by name but wasn't installed in node_modules",
      );
      return;
    }
    const filePath = pathGetter(resolve(pluginDir, buildDirName));

    let cjsPath: string | undefined = cjsPathGetter?.(resolve(pluginDir, buildDirName));
  ...
  async updatePlugin(pluginName: string): Promise<boolean> {
    ...
    if (!pluginName || !pluginPckgOld?.version)
    ...
    await runShellCommand(
      `yarn upgrade ${pluginName}@${updateInfo.packageVersion} --exact --non-interactive`,
      process.cwd(),
    );
    await sleep(1);
  ...
  async installPlugin(pluginName: string): Promise<boolean> {
    ...
    if (!pluginName || !info || !info.packageVersion || !info.version)
      throw new HttpException('Plugin was not found', HttpStatus.METHOD_NOT_ALLOWED);
    ...
    const pluginPckgNew = await getModulePackage(pluginName);
    if (!pluginPckgNew?.version)
      throw new HttpException('Plugin package was not found', HttpStatus.INTERNAL_SERVER_ERROR);
  ...
  public async activatePlugin(pluginName: string): Promise<boolean> {
    ...
    if (!pluginPckg?.version || !pluginPath)
      throw new HttpException(
        'Failed to find package.json of the plugin ' + pluginName,
