...
export class ThemeService {
   ...
   */
  public mergePages(
    themeConfig?: TPageConfig,
    userConfig?: TPageConfig,
    globalThemeMods?: TCromwellBlockData[],
    globalUserMods?: TCromwellBlockData[],
  ): TPageConfig {
    // Merge global mods
  ...
  public getPageConfigFromThemeConfig(
    themeConfig: TThemeConfig | undefined | null,
    pageRoute: string,
    pageId?: string,
  ): TPageConfig | undefined {
  ...
  public async getPageConfig(
    ...
    const themePageConfig: TPageConfig | undefined = this.getPageConfigFromThemeConfig(
      themeConfig,
      pageRoute,
      userPageConfig?.id,
    );
    ...
    const pageConfig = this.mergePages(
      themePageConfig,
      userPageConfig,
      themeConfig?.globalModifications,
      userConfig?.globalModifications,
    );
    return pageConfig;
  }
  ...
  public async saveUserPageConfig(userPageConfig: TPageConfig, themeName: string): Promise<boolean> {
    ...
    if (!userPageConfig.modifications) {
      logger.error(
        'Server::saveUserPageConfig: Invalid userPageConfig, no modifications',
        JSON.stringify(userPageConfig),
      );
    ...
    const oldUserPageConfig: TPageConfig | undefined = this.getPageConfigFromThemeConfig(
      userConfig,
      userPageConfig.route,
      userPageConfig.id,
    );
      ...
      if (!mod.global && userConfig.globalModifications) {
        for (const globalMod of userConfig.globalModifications) {
          if (mod.id === globalMod.id) {
            userConfig.globalModifications = userConfig.globalModifications?.filter(
              (filteredMod) => filteredMod.id !== mod.id,
            );
          }
        }
      }
  ...
  public async getThemePalette(themeName: string): Promise<TPalette> {
    ...
    return Object.assign({}, configs.themeConfig?.palette, configs.userConfig?.palette);
  }
  ...
  public async deletePage(pageRoute: string, themeName: string): Promise<boolean> {
    ...
    if (!page) throw new HttpException('Page was not found by pageRoute', HttpStatus.NOT_ACCEPTABLE);

    if (!page.isVirtual) throw new HttpException('Page cannot be deleted', HttpStatus.NOT_ACCEPTABLE);
  ...
  public async activateTheme(themeName: string): Promise<boolean> {
    ...
    if (!themePckg?.version || !themePath)
      throw new HttpException(
        'Failed to find package.json of the theme ' + themeName,
  ...
  async updateTheme(themeName: string): Promise<boolean> {
    ...
    if (!themeName || !pckgOld?.version)
      throw new HttpException('Theme package not found', HttpStatus.INTERNAL_SERVER_ERROR);
    const oldVersion = pckgOld.version;

    const updateInfo = await this.checkThemeUpdate(themeName);
    if (!updateInfo || !updateInfo.packageVersion)
      throw new HttpException('No update available', HttpStatus.METHOD_NOT_ALLOWED);
    if (updateInfo.onlyManualUpdate)
      throw new HttpException(
    ...
    await runShellCommand(
      `yarn upgrade ${themeName}@${updateInfo.packageVersion} --exact --non-interactive`,
      process.cwd(),
    );
    await sleep(1);
    ...
    const settings = await getCmsSettings();
    if (settings?.themeName === themeName) updateInfo.restartServices.push('renderer');
    ...
    if ((updateInfo?.restartServices ?? []).includes('api-server')) {
      ...
      if (resp1.message !== 'success') {
        ...
        if (resp2.message !== 'success')
          throw new HttpException('Could not apply new server with new plugin', HttpStatus.INTERNAL_SERVER_ERROR);
  ...
  async installTheme(themeName: string): Promise<boolean> {
    ...
    if (!themeName || !info || !info.packageVersion || !info.version)
      throw new HttpException('Theme was not found', HttpStatus.METHOD_NOT_ALLOWED);
  ...
  private async deleteTheme(themeName: string): Promise<boolean> {
    const pckgOld = await getModulePackage(themeName);
    const oldVersion = pckgOld?.version;
    if (!themeName || !oldVersion)
      throw new HttpException('Plugin package not found', HttpStatus.INTERNAL_SERVER_ERROR);
