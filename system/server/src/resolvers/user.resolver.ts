...
export class UserResolver {
  ...
  @Query(() => User)
  async [getOneByIdPath](@Ctx() ctx: TGraphQLContext, @Arg('id', () => Int) id: number): Promise<TUser> {
    return getByIdWithFilters('User', ctx, ['read_users', 'read_my_user'], ['read_users'], id, async (id) => {
      // If no 'read_user', check that has 'read_my_user' and the same id
      if (!matchPermissions(ctx.user, ['read_users'])) {
        if (ctx?.user?.id + '' !== id + '') throw new HttpException('Access denied.', HttpStatus.FORBIDDEN);
      }
  ...
  async [updatePath](
    ...
    return updateWithFilters('User', ctx, ['update_user', 'update_my_user'], data, id, async (id, data) => {
      ...
      if (
        data?.roles?.length &&
        (data?.roles?.length !== oldUser?.roles?.length ||
  ...
  async [deletePath](@Ctx() ctx: TGraphQLContext, @Arg('id', () => Int) id: number): Promise<boolean> {
    return deleteWithFilters('User', ctx, ['delete_user'], id, (...args) => this.repository.deleteUser(...args));
  }
