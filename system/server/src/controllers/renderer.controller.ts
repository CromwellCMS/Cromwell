...
export class RendererController {
  private rendererService = getDIService(RendererService);

  @Get('page')
  @AuthGuard({ permissions: ['all'] })
  @ApiOperation({
  ...
  async getRendererData(
    ...
    if (!pageName) throw new HttpException('Page name is not valid: ' + pageName, HttpStatus.NOT_ACCEPTABLE);
    if (!themeName) throw new HttpException('Theme name is not valid: ' + themeName, HttpStatus.NOT_ACCEPTABLE);
  ...
  async purgePageCache(@Query('pageRoute') pageRoute: string) {
    ...
    if (!pageRoute) throw new HttpException('Page route is not valid: ' + pageRoute, HttpStatus.NOT_ACCEPTABLE);
