 ...
 */
export const pluginsDataFetcher = async (
  pageName?: TDefaultPageName | string,
  context?: TStaticPageContext,
  ...
  const promises = configPlugins.map(async (data) => {
    ...
    if (!plugin) {
      ...
      try {
        ...
        console.error(
          `[Error] pluginsDataFetcher: Failed to require plugin Externals: ${pluginName} at ${pluginCjsPath.metaPath}`,
          e,
        );
      }
      ...
      try {
        ...
        console.error(
          `[Error] pluginsDataFetcher: Failed to require plugin: ${pluginName} at ${pluginCjsPath.cjsPath}`,
          e,
        );
      }
