...
if (isServer()) {
  if (!global.CromwellStore) global.CromwellStore = initialStore;
} else {
  if (!window.CromwellStore) window.CromwellStore = initialStore;
}
...
export const onStoreChange = <K extends keyof TCromwellStore>(
  itemName: K,
...
export const getSharedComponent = <K extends keyof TSharedComponents>(componentName: K): TSharedComponents[K] => {
  const components = getStore().components;
  return components ? components[componentName] : undefined;
};

export const registerSharedComponent = <K extends keyof TSharedComponents>(
  componentName: K,
  component: TSharedComponents[K],
): void => {
...
export const getBlockInstance = <TContentBlock = React.Component>(
  blockId: string,
): TCromwellBlock<TContentBlock> | undefined => {
