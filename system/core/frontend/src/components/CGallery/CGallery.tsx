...
export class CGallery extends React.Component<TCGalleryProps> {
  ...
  private onActiveSlideChange = (index: number) => {
    ...
    if (this.gallerySettings?.thumbs && this.gallerySettings?.images?.length) this.forceUpdateThumbs?.();
  };
  ...
  private getContent = (data?: TCromwellBlockData, width?: number, height?: number) => {
    ...
    const ButtonBackContent =
      gallerySettings.components?.backButton ??
      (() => <div className={clsx(styles.navBtnContent, styles.btnBack)}></div>);
    const ButtonNextContent =
      gallerySettings.components?.nextButton ??
      (() => <div className={clsx(styles.navBtnContent, styles.btnNext)}></div>);
    ...
    if (width && gallerySettings.slideMinWidth && !gallerySettings.visibleSlides) {
      ...
      if (gallerySettings.slideMaxWidth && slideWidth > gallerySettings.slideMaxWidth)
        slideWidth = gallerySettings.slideMaxWidth;
      ...
      >
        ...
        >
          ...
          >
              ...
              gallerySettings.images.map((img, index) => {
                ...
                let imgItem = (
                  <Image
                    ...
                    alt={img.alt}
                    className={clsx(
                      gallerySettings?.backgroundSize === 'contain' ? styles.slideContain : styles.slideCover,
                    )}
                  />
                );
                ...
                if (img.href) {
                  imgItem = <Link href={img.href}>{imgItem}</Link>;
                }
                imgItem = (
                  <Slide
            ...
            {gallerySettings.slides &&
              gallerySettings.slides.map((slideJsx, index) => {
                let el = slideJsx;
                if (gallerySettings.components?.imgWrapper) {
                  const WrapComp = gallerySettings.components.imgWrapper;
          ...
          {gallerySettings.navigation && (
            <>
              <ButtonBack className={clsx(styles.navBtn, styles.navBtnBack, gallerySettings.classes?.navBtn)}>
                <ButtonBackContent />
              </ButtonBack>
              <ButtonNext className={clsx(styles.navBtn, styles.navBtnNext, gallerySettings.classes?.navBtn)}>
                <ButtonNextContent />
              </ButtonNext>
            </>
          )}
      ...
      <>
        ...
        {!!(gallerySettings?.thumbs && gallerySettings?.images?.length) && (
          <Thumbs
            ...
            getUpdate={(forceUpdate) => {
              this.forceUpdateThumbs = forceUpdate;
            }}
          />
        )}
    ...
    return (
      <CBlock
        ...
        content={(data, ref, setContentInstance) => {
          ...
          return (
            <ReactResizeDetector handleWidth handleHeight refreshMode="throttle" refreshRate={50}>
              {({ targetRef, width, height }) => {
                return (
                  <div id={this.galleryId} className={styles.max} ref={targetRef as any}>
...
/** @internal */
export function CarouselOnChangeWatcher(props: { onChange: (index: number) => void }) {
