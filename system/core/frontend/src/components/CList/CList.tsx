...
export class CList<DataType, ListItemProps = any>
  ...
  implements TCList<DataType, ListItemProps>
{
  ...
  public onPageScrolled = (pageNumber: number) => {
    ...
    if (props.useQueryPagination) {
      ...
      if (currentPageNumber !== pageNumber) window.history.pushState({}, '', getPagedUrl(pageNumber));
    }
  };
  ...
  public openPage = async (pageNumber: number) => {
    ...
    if (this.currentPageNum !== pageNumber) {
      ...
      if (props.disableCaching) {
        this.dataList = [];
        this.pageStatuses = this.pageStatuses.map((s) => (s === 'fetched' || s === 'failed' ? 'deferred' : s));
      }
  ...
  render() {
    ...
    if (this.list.length === 0) {
      content = props.noDataLabel ? <h3>{props.noDataLabel}</h3> : <></>;
      return this.wrapContent(content);
    }
    ...
    content = (
      <>
        <div
          className={`${styles.scrollBox} ${props.cssClasses?.scrollBox ?? ''}`}
        ...
        >
          <div className={`${styles.wrapper} ${props.cssClasses?.contentWrapper ?? ''}`} ref={this.wrapperRef}>
            ...
            {props.useAutoLoading && (
              <div style={{ display: 'none' }} ref={this.throbberAutoloadingAfter}>
                <div className={styles.throbberAutoloading} dangerouslySetInnerHTML={{ __html: throbber }}></div>
              </div>
            )}
          </div>
        </div>
        {props.useShowMoreButton && !this.isPageLoading && this.maxPage > this.maxPageBound && (
          <div className={styles.showMoreBtnContainer}>
            {props.elements?.showMore ? (
              ...
              <div className={styles.showMoreBtn} onClick={this.handleShowMoreClick}>
                Show more
              </div>
            )}
          </div>
