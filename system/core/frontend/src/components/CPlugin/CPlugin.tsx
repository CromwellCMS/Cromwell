      ...
      <CBlock
        ...
        content={(data, blockRef, setContentInstance) => {
          ...
          if (name && !component) {
            ...
            const loader =
              getStoreItem('environment')?.isAdminPanel && props.adminPanel !== false
                ? restAPIClient?.getPluginAdminBundle
                : restAPIClient?.getPluginFrontendBundle;
          ...
          if (PluginComponent && isValidElementType(PluginComponent))
            return (
              <ErrorBoundary>
                <PluginComponent
                  blockId={data?.id}
