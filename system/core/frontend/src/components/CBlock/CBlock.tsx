  ...
  implements TCromwellBlock<TContentBlock>
{
  ...
  public rerender = () => {
    ...
    if (!this.rerenderPromise)
      this.rerenderPromise = new Promise((done) => {
  ...
  private getVirtualBlock = (b: TCromwellBlockData): JSX.Element => {
    ...
    if (b.type === 'HTML') {
      return <CHTML {...defProps} />;
    }
    if (b.type === 'image') {
      return <CImage {...defProps} />;
    }
    if (b.type === 'container') {
      return <CContainer {...defProps} />;
    }
    if (b.type === 'text') {
      return <CText {...defProps} />;
    }
    if (b.type === 'gallery') {
      return <CGallery {...defProps} />;
    }
    if (b.type === 'plugin') {
      return <CPlugin {...defProps} />;
    }
    if (b.type === 'editor') {
      return <CEditor {...defProps} />;
    }

    return <CBlock {...defProps} />;
  };
  ...
  public getDefaultContent(setClasses?: (classes: string) => void): React.ReactNode | null {
    ...
    const jsxChildren = this.props.content?.(
      this.data,
      this.blockRef,
  ...
  public contentRender(
    getContent?: TBlockContentProvider['getter'] | null,
    className?: string,
    ...
    return (
      <div
        id={this.htmlId}
        key={this.hasBeenMoved ? this.htmlId + '_moved' : this.htmlId + '_orig'}
  ...
  public consumerRender(): JSX.Element | null {
    return (
      <BlockContentConsumer key={this.htmlId + '_consumer'}>
        {(content) => {
  ...
  render(): React.ReactNode | null {
    ...
    return (
      <BlockStoreConsumer key={this.htmlId + '_render'}>
        {(value) => {
          ...
          if (
            this.pageInstances &&
            (!this.pageInstances[this.props.id] ||
