...
export class Importer implements Required<TCromwellNodeModules> {
  ...
  public importModule(moduleName, namedExports = ['default']): Promise<boolean> | boolean {
    ...
    }${bundledModulesDirName}/${moduleName}/${moduleMetaInfoFileName}`;
    const importerFilepath = `${
      this.prefix ? `/${this.prefix}` : ''
    }/${bundledModulesDirName}/${moduleName}/${moduleMainBuildFileName}`;
    const importerEntireLibFilepath = `${
      this.prefix ? `/${this.prefix}` : ''
    }/${bundledModulesDirName}/${moduleName}/${moduleLibBuildFileName}`;
   ...
   */
  private serverImport(options: { moduleName: string; metaFilepath: string; importerNodeFilepath: string }) {
    ...
    try {
      ...
      try {
        const fullPath = this.resolve(
          this.serverPublicDir ? this.serverPublicDir : this.resolve(process.cwd(), 'public'),
          metaFilepath,
        );
  ...
  private browserImport = async (options: {
    ...
    const { moduleName, moduleVer, metaFilepath, namedExports, importerEntireLibFilepath, importerFilepath } = options;
    ...
    const useImporter = async (namedExport: string): Promise<boolean> => {
      ...
      try {
        ...
        console.error(
          `Cromwell:importer: An error occurred while loading the library: import { ${namedExport} } from '${moduleName}'`,
          error,
        );
        return false;
      }
    ...
    if (!this.importStatuses[moduleName]) {
      ...
      try {
        ...
        this.log(
          `Cromwell:importer: isLibImport:`,
          isLibImport,
          'Cromwell?.modules?.[moduleName]',
          this.modules?.[moduleName],
          ' Cromwell.imports[moduleName] ',
      ...
      if (success.includes(false)) {
        ...
        this.log(
          'Cromwell:importer: All initially requested named exports for module "' +
            moduleName +
  ...
  public async importScriptExternals(metaInfo: TScriptMetaInfo | undefined): Promise<boolean> {
    ...
    if (typeof externals === 'object' && Object.keys(externals).length > 0) {
      ...
      this.log(
        `Cromwell:importer: ${successNum}/${Object.keys(externals).length} externals for module ${
          metaInfo.name
        } have been loaded`,
      );
