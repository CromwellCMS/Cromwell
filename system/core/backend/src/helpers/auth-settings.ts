...
export const getAuthSettings = async (options?: { serverCachePath?: string }): Promise<TAuthSettings> => {
  ...
  authSettings = {
    accessSecret:
      cmsConfig.accessTokenSecret ??
      cachedSettings?.accessSecret ??
      cryptoRandomString({ length: 8, type: 'ascii-printable' }),
    refreshSecret:
      cmsConfig.refreshTokenSecret ??
      cachedSettings?.refreshSecret ??
      cryptoRandomString({ length: 8, type: 'ascii-printable' }),
    cookieSecret:
      cmsConfig.cookieSecret ?? cachedSettings?.cookieSecret ?? cryptoRandomString({ length: 8, type: 'url-safe' }),
    serviceSecret:
