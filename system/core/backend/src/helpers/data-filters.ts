...
type TDataFilterParameters<TEntity = TBasePageEntity, TCreateEntity = TBasePageEntityInput,
    TUpdateEntity = TBasePageEntityInput, TEntityFilter = TBaseFilter> = {
        ...
        'getManyOutput': { data: TPagedList<TEntity>; params?: TPagedParams<TEntity>; filter?: TEntityFilter } & TFilterFunctionBaseOptions;
        ...
        'deleteManyOutput': { input: TDeleteManyInput; filter?: TEntityFilter; success: boolean } & TFilterFunctionBaseOptions;
...
export type TFilterableEntities = {
    ...
    ProductCategory: { entity: TProductCategory; create: TProductCategoryInput; update: TProductCategoryInput; filter: TProductCategoryFilter };
    ProductReview: { entity: TProductReview; create: TProductReviewInput; update: TProductReviewInput; filter: TProductReviewFilter };
    ...
    CustomEntity: { entity: TCustomEntity; create: TCustomEntityInput; update: TCustomEntityInput; filter: TCustomEntityFilter };
    Coupon: { entity: TCoupon; create: TCouponInput; update: TCouponInput; filter: TBaseFilter };
}

type TGetDataFilterParameter<TEntityKey extends keyof TFilterableEntities, TAction extends keyof TDataFilterParameters> = TDataFilterParameters<
...
type TRegisterDataFilterOptions<TEntityKey extends keyof TFilterableEntities, TAction extends keyof TDataFilterParameters> = {
    id: string;
    entity: TEntityKey | '*';
    action: TAction;
...
const dataFilters: Record<string, TRegisterDataFilterOptions<keyof TFilterableEntities, keyof TDataFilterParameters>> = {};

export const registerDataFilter = <TEntityKey extends keyof TFilterableEntities, TAction extends keyof TDataFilterParameters>(
    options: TRegisterDataFilterOptions<TEntityKey, TAction>) => {
    dataFilters[options.id] = options as any;
}

const geTDataFilterParameters = <TEntityKey extends keyof TFilterableEntities, TAction extends keyof TDataFilterParameters>(
    entity: TEntityKey | '*', action: TAction
): (TDataFilterFunction<TEntityKey, TAction>)[] => {
    return Object.values(dataFilters).filter(options => {
...
export const applyDataFilters = async <TEntityKey extends keyof TFilterableEntities, TAction extends keyof TDataFilterParameters>(
    entity: TEntityKey, action: TAction,
    data: TGetDataFilterParameter<TEntityKey, TAction>): Promise<TGetDataFilterParameter<TEntityKey, TAction>> => {
