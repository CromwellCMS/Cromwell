...
export const getNodeModuleVersion = (moduleName: string, importFromPath?: string): string | undefined => {
  ...
  try {
    modulePackageJson = importFromPath ? (importFrom(importFromPath, pckgImportName) as any) : require(pckgImportName);
  } catch (e) {
    console.log(
      `Cromwell bundler: Failed to require package ${pckgImportName} ${
        importFromPath ? 'from ' + importFromPath : ''
      }. You may need to reconfigure your frontendDependencies`,
    );
  }
...
export const getModuleInfo = (moduleName: string, moduleVer?: string, from?: string): TModuleInfo => {
  ...
  if (!modulesExportKeys[moduleName]) {
    ...
    try {
      const modulePackageJson: TPackageJson | undefined = from
        ? (importFrom(from, `${moduleName}/package.json`) as any)
 ...
 */
const collectDeps = (packageDeps: Record<string, string>, packagePath: string, store: TDependency[]): TDependency[] => {
...
const hoistDeps = (
  ...
  for (const module of store) {
    ...
    const versions = Object.entries(module.versions)
      .sort((a, b) => b[1] - a[1])
      .map((entry) => entry[0]);
    ...
    packages.forEach((pkg, i) => {
      if (pkg.name == module.name) localPckIndex = i;
    });
        ...
        Object.entries(module.packages).forEach(([packagePath, packageVer]) => {
          if (packageVer === ver) {
            ...
            if (!forceInstall) {
              console.log(
                colors.brightRed(
                  `Cromwell:: Error. Abort operation. Please fix "${module.name}": "${ver}" or run command in force mode (add -f flag).\n`,
                ),
              );
              process.exit();
            }
...
export const globPackages = async (projectRootDir: string): Promise<string[]> => {
  ...
  await new Promise((done) => {
    asyncEach(
      packageGlobs,
      function (pkgPath: string, callback: () => void) {
...
export const hoistDependencies = (packages: TPackage[], isProduction, forceInstall): TGetDeps => {
  ...
  const hoistedDependencies: THoistedDeps = JSON.parse(
    JSON.stringify(hoistDeps(dependencies, packages, isProduction, forceInstall)),
  );
  const hoistedDevDependencies: THoistedDeps = JSON.parse(
    JSON.stringify(hoistDeps(devDependencies, packages, isProduction, forceInstall)),
  );
...
export const parseFrontendDeps = async (
  dependencies: (string | TFrontendDependency)[],
...
export const collectFrontendDependencies = async (
  packages: TPackage[],
  forceInstall?: boolean,
): Promise<TFrontendDependency[]> => {
  ...
  packages.forEach((pckg) => {
    ...
    if (pckheDeps && Array.isArray(pckheDeps)) {
      pckheDeps.forEach((dep) => {
        ...
        const frontendDep: TFrontendDependency =
          typeof dep === 'object'
            ? dep
            : {
                name: dep,
        ...
        frontendDependencies.every((mainDep, i) => {
          if (mainDep.name === frontendDep.name) {
            if (mainDep.version !== frontendDep.version) {
              ...
              if (!forceInstall) {
                console.log(
                  colors.brightRed(`Cromwell:: Error. Please fix "${frontendDep.name}": "${frontendDep.version}"\n`),
                );
              }
            } else {
              index = i;
...
export const defaultFrontendDeps: (string | TFrontendDependency)[] = [
  ...
  'react-resize-detector',
  {
    ...
    bundledCss: ['pure-react-carousel/dist/react-carousel.es.css'],
  },
  {
    name: 'react-image-lightbox',
    bundledCss: ['react-image-lightbox/style.css'],
  },
];
