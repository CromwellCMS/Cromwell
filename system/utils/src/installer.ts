...
export const installer = async (
  ...
  const main = async () => {
    ...
    const { hoistedDependencies, hoistedDevDependencies } = hoistDependencies(packages, isProduction, forceInstall);
    ...
    asyncEach(
      ...
      (path: string, callback: () => void) => {
        console.log(
          colors.cyan(
            `\nCromwella:: Installing modules for: ${colors.brightCyan(`${path}`)} package in ${colors.brightCyan(
              `${installationMode}`,
            )} mode...\n`,
          ),
        );
