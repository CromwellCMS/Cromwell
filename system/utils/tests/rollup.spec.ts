...
describe('rollup', () => {
  it('creates plugin options', async () => {
    ...
    await fs.outputFile(join(testDir, 'src/admin/index.js'), `export default () => 'test';`);
    await fs.outputFile(join(testDir, 'src/frontend/index.js'), `export default () => 'test';`);
    await fs.outputFile(join(testDir, 'src/backend/index.js'), `class TestEnt {}; export default TestEnt;`);
    await fs.outputFile(
  ...
  it('creates theme options', async () => {
    ...
    await fs.outputJSON(join(testDir, 'package.json'), pckg);
    await fs.outputFile(join(testDir, 'src/pages/index.js'), `export default () => 'test';`);
