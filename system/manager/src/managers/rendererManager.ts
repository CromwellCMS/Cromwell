...
export const startRenderer = async (
  command?: TRendererCommands,
  options?: {
    port?: string | number;
  },
): Promise<boolean> => {
  ...
  const themeName =
    cmsSettings?.themeName ??
    cmsSettings?.defaultSettings?.publicSettings?.themeName ??
  ...
  if (rendererEnv && rendererStartupPath) {
    const rendererProc = await startService({
      ...
      args: [rendererEnv, `--theme-name=${themeName}`, `--port=${port}`],
      command: rendererEnv,
...
const isThemeBuilt = async (dir: string): Promise<boolean> => {
  return (
    (await fs.pathExists(resolve(dir, '.next/static'))) &&
...
const pollPages = async (port: string | number, themeName: string) => {
  ...
  const promises = infos
    .filter((info) => !info.route.startsWith('pages/'))
    .map(async (info) => {
      for (let i = 0; i < 2; i++) {
...
const startRendererAliveWatcher = (
  command: TRendererCommands,
  options: {
    port: string | number;
  },
...
const rendererAliveWatcher = async (
  command: TRendererCommands,
  options: {
    port: string | number;
  },
