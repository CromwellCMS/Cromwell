...
export const buildTask = async (watch?: boolean, port?: string) => {
  ...
  if (moduleInfo.type !== 'theme' && moduleInfo.type !== 'plugin') {
    logger.error(
      '"type" property of cromwell config in package.json must have either "plugin" or "theme" value. Eg.:' +
        errorExample,
    );
    return;
  }
...
export const rendererBuildAndSaveTheme = async (themeModuleName: string): Promise<boolean> => {
  ...
  if (themeRollupBuildDir) {
    ...
    if (themeTempAdminBuildDir && adminBuildDir && (await fs.pathExists(themeTempAdminBuildDir))) {
...
function printTimings(timings: any) {
  ...
  Object.keys(timings).forEach((label) => {
    const appliedColor = label[0] === '#' ? (label[1] !== '#' ? underline : bold) : (text: string) => text;
