...
try {
  ...
  spawnSync(`npm i node-window-manager --no-save --no-package-lock`, {
    shell: true,
    cwd: __dirname,
    stdio: 'inherit',
  });
...
const winStart = () => {
  ...
  const monitor =
    monitorNum !== null ? windowManager.getMonitors()[monitorNum] : windowManager.getActiveWindow().getMonitor();
  ...
  const startTerminal = async (title, command, bounds, timeout) => {
    ...
    if (newPid && !isNaN(newPid)) {
      await saveProcessPid(title, process.pid, newPid);
  ...
  const winUpdateCycle = async () => {
    ...
    if (services.coreCommon) {
      await startTerminal(
        getCacheKey(cacheKeys.coreCommon),
        `cd ${projectRootDir}\\system\\core\\common && npm run ${services.coreCommon}`,
        {
    ...
    if (services.coreBackend) {
      await startTerminal(
        getCacheKey(cacheKeys.coreBackend),
        `cd ${projectRootDir}\\system\\core\\backend && npm run ${services.coreBackend}`,
        {
    ...
    if (services.coreFrontend) {
      await startTerminal(
        getCacheKey(cacheKeys.coreFrontend),
        `cd ${projectRootDir}\\system\\core\\frontend && npm run ${services.coreFrontend}`,
        {
          x: startX,
          y: coreStartY,
          height: coreHeigth,
    ...
    if (services.server) {
      // SERVER
      await startTerminal(
        getCacheKey(cacheKeys.server),
        `cd ${projectRootDir}\\system\\server && npm run ${services.server}`,
        {
          x: startX,
          y: monitorBounds.y,
    ...
    if (services.renderer) {
      // RENDERER
      await startTerminal(
        getCacheKey(cacheKeys.renderer),
        `cd ${projectRootDir}\\system\\renderer && npm run ${services.renderer}`,
        {
          x: startX,
    ...
    if (services.adminPanel) {
      // ADMIN PANEL
      await startTerminal(
        getCacheKey(cacheKeys.adminPanel),
        `cd ${projectRootDir}\\system\\admin && npm run ${services.adminPanel}`,
        {
    ...
    if (services.manager) {
      // MANAGER
      await startTerminal(
        getCacheKey(cacheKeys.manager),
        `cd ${projectRootDir}\\system\\manager && npm run ${services.manager}`,
        {
          x: startX + overlayShift,
          y: monitorBounds.y,
          height: monitorBounds.height * serverHeigth + padding,
    ...
    if (services.other) {
      ...
      for (const dir of otherDirs) {
        ...
        await startTerminal(
          getCacheKey(dir),
          `cd ${fullDir} && npm run ${services.other}`,
          {
