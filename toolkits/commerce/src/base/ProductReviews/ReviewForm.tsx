...
export const ReviewForm = ({ productId, notifier = getNotifier(), parentProps }: ReviewFormProps) => {
    ...
    return (
      <div className={clsx(styles.ReviewForm, classes?.ReviewForm)}>
        <Alert severity="success">
          {text.submitSuccess ??
            'Thank you! Your review will appear on this page after approval by the website moderator'}
        </Alert>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <TextField
        label={text.fieldTitleLabel ?? 'Title'}
        name="Title"
        variant="outlined"
        size="small"
        fullWidth
      ...
      <div className={clsx(styles.submitBtnWrapper, classes?.submitBtnWrapper)}>
        <Button variant="contained" color="primary" size="large" onClick={handleSubmit} disabled={isLoading}>
