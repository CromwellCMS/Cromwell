...
export const useProductLink = (product?: TProduct, getProductLink?: (product: TProduct) => string | undefined) => {
  ...
  if (!productBaseRoute && !getProductLink) {
    console.error(
      '@cromwell/toolkit-commerce: cannot resolve product link. Configure `defaultPages` in cromwell.config.js ' +
...
export const useCategoryLink = (
  ...
  getCategoryLink?: (category: TProductCategory) => string | undefined,
) => {
  const pageContext = useAppPropsContext();
  ...
  if (!categoryBaseRoute && !getCategoryLink) {
    console.error(
      '@cromwell/toolkit-commerce: cannot resolve category link. Configure `defaultPages` in cromwell.config.js ' +
        'https://cromwellcms.com/docs/development/theme-development/#default-pages' +
