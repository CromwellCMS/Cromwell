      ...
      <CContainer className={styles.postContent} id="blog-3">
        ...
        {post?.tags && (
          <CContainer className={postStyles.tagsBlock} id="blog_05">
            {post?.tags?.map((tag) => {
              return (
                <Link href={`/tag/${tag.slug}`} key={tag.id} className={postStyles.tag}>
                  {tag?.name}
                </Link>
              );
            })}
  ...
  const post =
    (typeof slug === 'string' &&
      (await client.getPostBySlug(slug).catch((error: TGraphQLErrorInfo) => {
        if (error.statusCode !== 404) console.error('BlogPostPage::getStaticProps', error);
      }))) ||
    null;
