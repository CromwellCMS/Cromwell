...
export const createPayment = async (input: TOrderPaymentSession) => {
  ...
  if (!input.successUrl) {
    logger.warn(
      `${pluginName}: input.successUrl wasn't set. Please provide valid frontend URL where order was created`,
    );
    return;
  }
  ...
  const convertPrice = (price: number | null | undefined) =>
    (parseFloat(store.convertPrice(price ?? 0, defaultCurrency, currency) as any) * 100).toFixed(2);
